...
import com.inditex.zarachallenge.infrastructure.in.kafka.KafkaListener;
...
public class ZaraInitialConfig implements CommandLineRunner {
	...
	public void startMockServer() {
		mockServer = new MockServer(3005);
		mockServerClient = new MockServerClient("localhost", 3005);
	}

	@Override
	public void run(String... args) throws Exception {
		Consumer<Message<ProductAvailablityEvent>> consumer = kafka.KafkaConsumer();
		try {
			List<ProductAvailablityEvent> stocks = Files
			...
			stocks.forEach(stock -> consumer.accept(new GenericMessage<ProductAvailablityEvent>(stock)));
		} catch (Exception e) {
	...
	public ProductAvailablityEvent convertStock(List<String> stock) {
		return ProductAvailablityEvent.builder().skuId(Long.parseLong(stock.get(0)))
